...
async def save_list_callback(callback: CallbackQuery, bot: Bot):
    """
    Обробляє запит на збереження списку.
    Використовує централізовану функцію process_and_save_list для всієї бізнес-логіки.
    """
    ...
    try:
        async with async_session() as session:
            async with session.begin():
                # --- ЗМІНА: Вся складна логіка тепер в одній функції ---
                main_list_path, surplus_list_path = await process_and_save_list(session, user_id)

        # --- Блок відправки файлів та меню (логіка не змінилась) ---
        await callback.message.delete()

        if not main_list_path and not surplus_list_path:
            # Це спрацює, якщо список був порожнім
        ...
        # Повертаємо початкове повідомлення, якщо сталася помилка
        ...
        # Прибираємо тимчасові файли, якщо вони були створені
